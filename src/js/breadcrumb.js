// Mobile-Optimized Breadcrumb Navigation for IVMS Website
// This script adds breadcrumb navigation with mobile-friendly features
...
(function() {
    ...
    function initializeBreadcrumb() {
        ...
        // Find navigation and insert breadcrumb after it
        const mainNav = document.querySelector('nav.main-nav') || document.querySelector('nav');
        if (!mainNav) return;

        const breadcrumbNav = createBreadcrumbNav();
        mainNav.parentNode.insertBefore(breadcrumbNav, mainNav.nextSibling);
        ...
        const breadcrumbConfig = {
            ...
            'contact.html': [
                { text: 'Home', url: 'index.html', icon: '🏠' },
                { text: 'Contact', current: true }
            ],
            'governments.html': [
                { text: 'Home', url: 'index.html', icon: '🏠' },
                { text: 'Governments', current: true }
            ],
            'speedsense.html': [
                { text: 'Home', url: 'index.html', icon: '🏠' },
                { text: 'Solutions', url: 'index.html#solutions' },
                { text: 'SpeedSense', current: true }
            ],
            'logistics.html': [
                { text: 'Home', url: 'index.html', icon: '🏠' },
                { text: 'Solutions', url: 'index.html#solutions' },
                { text: 'Logistics', current: true }
            ],
            'rental-leasing.html': [
                { text: 'Home', url: 'index.html', icon: '🏠' },
                { text: 'Solutions', url: 'index.html#solutions' },
                { text: 'Rental & Leasing', current: true }
            ]
        };
        ...
        breadcrumbs.forEach((item, index) => {
            ...
            if (item.current || isLast) {
                ...
                breadcrumbHTML += `
                    <li class="breadcrumb-item" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">
                        <a href="${item.url}" class="breadcrumb-link" itemprop="item" aria-label="Navigate to ${item.text}">
                            <span itemprop="name">${item.icon ? item.icon + ' ' : ''}${item.text}</span>
                        </a>
                        <meta itemprop="position" content="${index + 1}" />
                    </li>
                `;
    ...
    function addTouchFeedback() {
        ...
        breadcrumbLinks.forEach(link => {
            ...
            link.addEventListener('touchstart', function() {
                this.style.backgroundColor = 'rgba(0, 78, 152, 0.15)';
            });
